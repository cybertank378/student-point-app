...
const emptyToNull = <T extends z.ZodTypeAny>(schema: T) => z.preprocess((val) => (val === "" ? null : val), schema);
...
export const EmailSchema = emptyToNull(z.string().regex(EMAIL_REGEX, "Format email tidak valid").nullable().optional());

export const PhotoSchema = emptyToNull(z.string().trim().nullable().optional());
...
export const TeacherObjectSchema = z.object({
  ...
  major: emptyToNull(z.string().min(2).nullable().optional()),
  ...
  civilServantRank: CivilServantRankEnum.nullable().optional(),

  roles: z.array(TeacherRoleEnum).min(1),

  isPns: z.boolean().default(false),

  homeroomClassIds: z.array(z.string().uuid()).optional().default([]),
});
...
export const teacherRefinement = (data: Partial<z.infer<typeof TeacherObjectSchema>>, ctx: z.RefinementCtx) => {
  if (data.isPns === true) {
...
export const CreateTeacherSchema = TeacherObjectSchema.superRefine(teacherRefinement);

/* ============================================================
   UPDATE SCHEMA (PARTIAL SAFE)
============================================================ */

export const UpdateTeacherSchema = TeacherObjectSchema.partial().superRefine(teacherRefinement);
