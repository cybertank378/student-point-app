...
function matchApiRoute(path: string): keyof typeof rbacConfig.api | undefined {
  return (
    Object.keys(rbacConfig.api) as Array<keyof typeof rbacConfig.api>
...
export function evaluatePolicy({ path, method, role }: PolicyInput): boolean {
  ...
  if (path.startsWith("/dashboard")) {
    const required =
      rbacConfig.dashboard[path as keyof typeof rbacConfig.dashboard];
    ...
    if (base) {
      ...
      const required =
        routePolicy[method.toUpperCase() as keyof typeof routePolicy];
