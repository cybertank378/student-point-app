...
import { getAvatarMenuByRole, roleConfig } from "@/security/rbacConfig";
    ...
    <header className="sticky top-0 z-40 h-16 bg-white border-b border-gray-200 shadow-sm">
      <div className="h-full px-4 md:px-8 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          <button onClick={onMenuClick} className="md:hidden text-gray-600 shrink-0">
            <MdMenu size={22} />
          </button>
          ...
          <div className="flex items-center gap-2 px-3 h-10 border border-gray-300 rounded-lg bg-white focus-within:border-indigo-500 transition-colors flex-1 min-w-0">
            ...
            <input type="text" placeholder="Search" className="flex-1 bg-transparent outline-none text-sm placeholder-gray-400 min-w-0" />
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 shrink-0">
          ...
          <div className="relative z-50" ref={dropdownRef}>
            <button type="button" onClick={() => setIsOpen((prev) => !prev)} className="w-8 h-8 rounded-full overflow-hidden">
              <Image src="/assets/images/avatar.png" alt="User Avatar" width={32} height={32} className="rounded-full object-cover" />
            </button>
              ...
              <div className="absolute right-0 mt-4 w-64 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden">
                ...
                <div className="flex items-center gap-3 px-4 py-4 border-b border-gray-100">
                  <div className="relative w-10 h-10 rounded-full overflow-hidden">
                    <Image src="/assets/images/avatar.png" alt="User" fill className="object-cover" />
                    <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-white rounded-full"></span>
                  </div>

                  <div>
                    <p className="text-sm font-semibold text-gray-800">{username || "User"}</p>
                    <p className="text-xs text-gray-500">{roleConfig[role].label}</p>
                  </div>
                </div>
