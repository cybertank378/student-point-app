...
import { generateSidebar } from "@/security/sidebar/getSidebarMenu";
...
export default function AppSidebar({ role, mobileOpen = false, onClose }: Props) {
  ...
  useEffect(() => {
    const parentIndex = menu.findIndex((item) => item.children?.some((child) => child.path && pathname.startsWith(child.path)));

    if (parentIndex !== -1) {
      setExpandedIndex(parentIndex);
    } else if (pathname === "/dashboard") {
      const masterIndex = menu.findIndex((item) => item.label === "Master Akademik");
        ...
        <div className="h-16 flex items-center px-4 border-b">
          ...
          <div className="flex items-center gap-3 flex-1">
            <Image
              ...
              width={40}
              height={40}
              className={`shrink-0 ${collapsed ? "mx-auto cursor-pointer" : ""}`}
              onClick={() => {
            ...
            {!collapsed && (
              <div className="flex flex-col leading-tight">
                <span className="text-white font-semibold text-sm">SMP Negeri 29</span>
                <span className="text-indigo-200 text-xs">Jakarta</span>
              </div>
            )}
          </div>
          ...
          {!collapsed && (
            <button onClick={toggleCollapse} className="text-indigo-200 hover:text-white transition">
              <ArrowMenuIcon />
            </button>
          )}
        </div>
